#!/usr/bin/env bash
# `make start` / `make restart-build` preflight (spec:
...
#    that takes the whole stack down: machines.yaml is the fleet inventory
#    (gitignored, copied from config/machines.example.yaml, distributed out of
#    band), while self-identification is per-machine and lives in a gitignored
#    .env — so the moment the fleet file lands, every machine
...
#
# Run: bash scripts/preflight.sh   (wired into `make start` and `make restart-build`)
set -euo pipefail
cd "$(dirname "$0")/.."
...
# and stray whitespace/CR are stripped so `ATLAS_SELF="mac-one"` and a
...
if [[ -z "$SELF" ]]; then
  {
    ...
    echo "   The fleet file is shared across the fleet; naming yourself is per-machine (gitignored .env)."
